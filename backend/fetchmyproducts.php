...
// Prepare SQL query to fetch products posted by the logged-in user
$sql = "SELECT p.product_id, p.product_title, p.product_content, p.product_pic, p.price, p.date, p.user_id, 
        ...
        AND p.user_id = ?  -- Use a placeholder for security
        ORDER BY p.date DESC";

$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $logged_in_user_id); // Bind user_id as an integer
$stmt->execute();
